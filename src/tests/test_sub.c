...
START_TEST(sub_test_1) {
  ...
  s21_sub_matrix(&result1, &result2, &end);
...
START_TEST(sub_test_err_1) {
  ...
  // s21_print_matrix(&result);
  int flag = s21_sub_matrix(&result1, &result2, &end);
...
START_TEST(sub_test_err_2) {
  ...
  result2.matrix[1][0] = 2;
  // s21_print_matrix(&result);
  int flag = s21_sub_matrix(&result1, &result2, NULL);
...
Suite *s21_sub_matrix_suite(void) {
  Suite *s;
  s = suite_create("\033[33ms21_sub_matrix\033[0m");

  TCase *tc_core;
  tc_core = tcase_create("s21_sub_matrix");
  tcase_add_test(tc_core, sub_test_1);
  tcase_add_test(tc_core, sub_test_2);

  tcase_add_test(tc_core, sub_test_err_1);
  tcase_add_test(tc_core, sub_test_err_2);
